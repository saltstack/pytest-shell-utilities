...
class SubprocessImpl:
    ...
    factory: "Union[Factory, Subprocess, ScriptSubprocess]" = attrs.field()

    _terminal: "Optional[subprocess.Popen[Any]]" = attrs.field(repr=False, init=False, default=None)
    _terminal_stdout: "Optional[SpooledTemporaryFile[bytes]]" = attrs.field(
        repr=False, init=False, default=None
    )
    _terminal_stderr: "Optional[SpooledTemporaryFile[bytes]]" = attrs.field(
        repr=False, init=False, default=None
    )
    _terminal_result: Optional[ProcessResult] = attrs.field(repr=False, init=False, default=None)
    _terminal_timeout: Union[int, float] = attrs.field(repr=False, init=False, default=None)
    _children: List[psutil.Process] = attrs.field(repr=False, init=False, factory=list)
...
class Factory:
    ...
    cwd: pathlib.Path = attrs.field(converter=resolved_pathlib_path)
    environ: EnvironDict = attrs.field(repr=False)
    slow_stop: bool = attrs.field(default=True)
    system_encoding: str = attrs.field(repr=False)
    timeout: Union[int, float] = attrs.field()

    impl: SubprocessImpl = attrs.field(repr=False, init=False)

    # Internal attributes
    _cmdline: List[Any] = attrs.field(repr=False, init=False, default=None)
...
class ScriptSubprocess(Subprocess):
    ...
    base_script_args: List[str] = attrs.field(factory=list)
...
class DaemonImpl(SubprocessImpl):
    ...
    factory: "Daemon" = attrs.field()

    _before_start_callbacks: List[Callback] = attrs.field(repr=False, hash=False, factory=list)
    _after_start_callbacks: List[Callback] = attrs.field(repr=False, hash=False, factory=list)
    _before_terminate_callbacks: List[Callback] = attrs.field(repr=False, hash=False, factory=list)
    _after_terminate_callbacks: List[Callback] = attrs.field(repr=False, hash=False, factory=list)
    _start_args_and_kwargs: StartDaemonCallArguments = attrs.field(
...
class Daemon(ScriptSubprocess):
    ...
    impl: DaemonImpl = attrs.field(repr=False, init=False)

    script_name: str = attrs.field()
    base_script_args: List[str] = attrs.field(factory=list)
    check_ports: List[int] = attrs.field(factory=list)
    stats_processes: "StatsProcesses" = attrs.field(repr=False, hash=False, default=None)
    start_timeout: Union[int, float] = attrs.field(repr=False)
    max_start_attempts: int = attrs.field(repr=False, default=3)
    extra_cli_arguments_after_first_start_failure: List[str] = attrs.field(hash=False, factory=list)
    listen_ports: List[int] = attrs.field(init=False, repr=False, hash=False, factory=list)
